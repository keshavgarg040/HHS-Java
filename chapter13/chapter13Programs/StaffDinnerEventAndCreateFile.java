...
public class StaffDinnerEventAndCreateFile {
	...
	ws.setEmployeeNumber(input.nextLine());
	System.out.print("Enter first name >> ");
	ws.setFirstName(input.nextLine());
	System.out.print("Enter last name >> ");
	ws.setLastName(input.nextLine());
	boolean isValidPayRate = false;
	while (!isValidPayRate) {
	    isValidPayRate = true;
	    try {
		System.out.print("Enter pay rate >> ");
		ws.setPayRate(input.nextDouble());
	    } catch (Exception e) {
	...
	bt.setEmployeeNumber(input.nextLine());
	System.out.print("Enter first name >> ");
	bt.setFirstName(input.nextLine());
	System.out.print("Enter last name >> ");
	bt.setLastName(input.nextLine());
	boolean isValidPayRate = false;
	while (!isValidPayRate) {
	    isValidPayRate = true;
	    try {
		System.out.print("Enter pay rate >> ");
		bt.setPayRate(input.nextDouble());
	    } catch (Exception e) {
		isValidPayRate = false;
	    }
	    input.nextLine();
	...
	co.setEmployeeNumber(input.nextLine());
	System.out.print("Enter first name >> ");
	co.setFirstName(input.nextLine());
	System.out.print("Enter last name >> ");
	co.setLastName(input.nextLine());
	boolean isValidPayRate = false;
	while (!isValidPayRate) {
	    isValidPayRate = true;
	    try {
		System.out.print("Enter pay rate >> ");
		co.setPayRate(input.nextDouble());
	    } catch (Exception e) {
		isValidPayRate = false;
	    }
	    input.nextLine();
	...
	Path file = Paths.get("C:\\Java\\Chapter.13\\Events.txt");
	try {
		...
		s = e[x].getEventNumber() + delimiter + e[x].getEventType() + delimiter + e[x].getGuests() + delimiter
			+ e[x].getPriceForEvent() + System.getProperty("line.separator");
